...
for item, device_detail in device_dict.items():
    ...
    for route, route_attribs in output[0]["result"]["vrfs"]["default"]["routes"].items():
        pprint(route)
        pprint(route_attribs["routeType"])
        if route_attribs["routeType"] == "static":
            for vias in route_attribs["vias"]:
                pprint("Next hop: " + vias["nexthopAddr"])
        pprint("---- Next route ----")
